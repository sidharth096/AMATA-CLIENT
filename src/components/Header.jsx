...
import { FaUser, FaShoppingCart, FaRupeeSign, FaSeedling, FaCookieBite, FaLeaf, FaBars, FaTimes, FaBookOpen, FaShoppingBag, FaFileAlt, FaChevronDown, FaChevronUp } from 'react-icons/fa'
...
const navLinks = [
  { label: 'Shop by Category', icon: <FaShoppingBag className="text-primary text-2xl" /> },
  { label: 'About', icon: <FaBookOpen className="text-primary text-2xl" /> },
  { label: 'Ayurvedic Remedies', icon: <FaShoppingBag className="text-primary text-2xl" /> },
  { label: 'Blog', icon: <FaFileAlt className="text-primary text-2xl" /> },
  { label: 'Connect', icon: <FaFileAlt className="text-primary text-2xl" /> },
]
    ...
    <header className="w-full border-b border-gray-100 bg-white sticky top-0 z-50 shadow-sm">
      ...
      <div className="w-full bg-white">
        <div className="max-w-7xl mx-auto flex items-center justify-between py-4 px-4 md:px-8">
          ...
          <nav className={`hidden lg:flex flex-1 items-center justify-center gap-8`}> 
            <div className="flex items-center gap-8 text-gray-700 font-medium">
              ...
              <div className="relative group">
                ...
                <div className="absolute left-0 mt-0 w-[700px] bg-white shadow-lg rounded-lg p-8 flex gap-12 z-30 border border-gray-100 min-h-[320px] opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity">
                  <div className="flex-1 flex flex-col gap-6">
                    {categories.slice(0, 6).map((cat, idx) => (
                      <div key={idx} className="flex items-center gap-4 hover:text-primary cursor-pointer">
                        <span className="flex items-center justify-center w-10 h-10 border rounded-full bg-gray-50">{cat.icon}</span>
              ...
              <div className="relative group">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 mt-0 w-48 bg-white shadow-lg rounded-lg py-6 px-6 flex flex-col gap-4 z-30 border border-gray-100 text-center opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity">
          ...
          <div className="flex items-center gap-6 absolute right-8 top-1/2 -translate-y-1/2 lg:static lg:translate-y-0">
            <FaUser className="text-2xl text-gray-700 cursor-pointer hover:text-primary transition-colors" />
            <FaShoppingCart className="text-2xl text-gray-700 cursor-pointer hover:text-primary transition-colors" />
          </div>
        </div>
      </div>
      {/* Mobile Sidebar Menu */}
      <div
        className={`fixed inset-0 z-40 bg-black/50 transition-opacity duration-300 md:hidden ${mobileMenuOpen ? 'block' : 'hidden'}`}
      ...
      >
        <div className="flex items-center justify-between px-4 py-4 border-b">
          <button className="text-2xl text-gray-700" onClick={() => setMobileMenuOpen(false)} aria-label="Close menu">
            <FaTimes />
          </button>
        ...
        <nav className="flex flex-col gap-2 mt-8 px-6">
          ...
          <button
            className="flex items-center gap-4 text-primary font-medium text-lg w-full focus:outline-none"
            ...
            <div className="pl-10 flex flex-col gap-3 py-2">
              {categories.map((cat, idx) => (
                <div key={idx} className="flex items-center gap-3 text-gray-700 text-base">
                  <span className="w-7 h-7 flex items-center justify-center border rounded-full bg-gray-50">{cat.icon}</span>
                  <span>{cat.label}</span>
                </div>
              ))}
            </div>
